...
export default {
	...
	methods: {
		...
		bindLogin() {
			...
			const data = {
				ddusername: this.account,
				password: this.password,
				from: util.shebei
			};
			// 去登录
			uni.request({
				url: util.apiurl + 'api/v1/login',
				method: 'POST',
				data: data,
				success: ret => {
					if (ret.statusCode == 200 && ret.data.code > 0) {
						// 存入用户信息
